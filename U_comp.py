...
def u_comp(circuit: QuantumCircuit, a_reg: QuantumRegister, b_reg: QuantumRegister, anc_ab, diff_reg: QuantumRegister | None = None):
    """
    ...
    # Optional: capture (b - a) low n bits to diff_reg before uncompute
    if diff_reg is not None:
        assert len(diff_reg) == n, "diff_reg must have same length as a_reg"
        for i in range(n):
            circuit.cx(b_extended[i], diff_reg[i])
